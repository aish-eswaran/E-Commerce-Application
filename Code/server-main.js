...
app.post('/viewProducts', function(req,res) {
	...
	if(asin && !keyword && !group) {
		mc.query('SELECT asin,productName FROM products WHERE asin=?',[asin],function(error,results,fields){
		if(error || results.length <= 0){
	...
	if(keyword && !asin && !group) {
		mc.query('SELECT asin,productName FROM products_read WHERE MATCH(productName, productDescription) AGAINST (? IN BOOLEAN MODE)',[newString],function(error,results,fields){
		if(error || results.length <= 0){
			return res.send({message: 'There are no products that match that criteria'});
		}
		return res.send({product: results});
	...
	if(asin && group && !keyword) {
		mc.query('SELECT asin,productName FROM products_read WHERE asin=? and groups=?',[asin,group],function(error,results,fields){
		if(error || results.length <= 0){
			return res.send({message: 'There are no products that match that criteria'});
		}
		return res.send({product: results});
	...
	if(!asin && keyword && group) {
		mc.query('SELECT asin,productName FROM products_read WHERE MATCH(productName, productDescription) AGAINST (? IN BOOLEAN MODE) and groups=?',[newString,group],function(error,results,fields){
		if(error || results.length <= 0){
			return res.send({message: 'There are no products that match that criteria'});
		}
		return res.send({product: results});
	...
	if(!asin && group && !keyword) {
		mc.query('SELECT asin,productName FROM products_read WHERE groups=?',[group],function(error,results,fields){
		if(error || results.length <= 0){
			return res.send({message: 'There are no products that match that criteria'});
		}
		return res.send({product: results});
	...
	if(!asin && !keyword && !group) {
		mc.query('SELECT asin,productName FROM products_read',function(error,results,fields){
		if(error || results.length <= 0){
			return res.send({message: 'There are no products that match that criteria'});
		}
		return res.send({product: results});
	...
	if(asin && keyword && group) {
		mc.query('SELECT asin,productName FROM products_read WHERE MATCH(productName, productDescription) AGAINST (? IN BOOLEAN MODE) and groups=? and asin=?',[newString,group,asin],function(error,results,fields){
		if(error || results.length <= 0){
			return res.send({message: 'There are no products that match that criteria'});
		}
		return res.send({product: results});

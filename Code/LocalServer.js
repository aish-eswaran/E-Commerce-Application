...
app.post('/viewProducts', function(req,res) {
	...
	qgroup = "%" + group + "%";
	if(!asin) {
		mc.query('SELECT asin,productName FROM products WHERE (productName like ? or productDescription like ?) and groups like ?',[qkeyword,qkeyword,qgroup],function(error,results,fields){
		if(error || results.length <= 0){
	...
	if(req.session && req.session.user) {
		...
		mc.query('SELECT * from users where username=?', [user], function(error, results, fields) {
			...
			if(results[0].is_admin == "Yes") {
							...
							mc.query('select b.productName as pname, count(a.asin) as quantity from purchaseHistory a, products b where a.user=? and a.asin=b.asin group by a.asin',[targetUser],function(error,results,fields){
								if(error || results.length <= 0){
									return res.send({message: 'There are no users that match that criteria'});
								}
...
app.post('/buyProducts', function(req, res) {
	if(req.session && req.session.user) {
		...
		var productsList = JSON.stringify(req.body.products);
		if(productsList.charAt(0) == '[') {
			productsList = productsList.slice(1,-1);
		}
		var prodList = productsList.split(',');
		
		var asinList = "";
		var total = 0;
		for(var i=0; i < prodList.length; i++) {
				var temp = prodList[i].split(':');
		...
		mc.query('SELECT checkValidProduct(?,?) as isValid',[asinList,total],function(error,results,fields){
			if(results[0].isValid == 0) {
				...
				mc.query('SELECT orderID from orderDetails where user=? and purchaseTime=?',[user,utcDate],function(error, results, fields) {
					...
					for(var i=0; i < prodList.length; i++) {
						var temp = prodList[i].split(':');
						var asin = temp[1].substring(1,temp[1].length-2);
						mc.query('INSERT into purchaseHistory values (?,?,?)',[user,asin,orderID],function(error, res, fields){
						if(error||res.length <= 0) {
							console.log("Error in inserting the product records");
						}

...
app.post('/viewProducts', function(req,res) {
	...
	qgroup = "%" + group + "%";
	if(!asin) {
		mc.query('SELECT asin,productName FROM products_read WHERE MATCH(productName, productDescription) AGAINST (? IN BOOLEAN MODE) and groups like ?',[newString,qgroup],function(error,results,fields){
		if(error || results.length <= 0){
